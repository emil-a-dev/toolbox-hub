  ...
  return (
    <ToolLayout title="Code Diff" description="Compare two code snippets and see the differences with syntax highlighting.">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div><label className="block text-sm font-medium text-gray-700 mb-1">Original</label><textarea value={codeA} onChange={(e) => setCodeA(e.target.value)} placeholder="Paste original code..." className="tool-textarea !min-h-[200px]" /></div>
        <div><label className="block text-sm font-medium text-gray-700 mb-1">Modified</label><textarea value={codeB} onChange={(e) => setCodeB(e.target.value)} placeholder="Paste modified code..." className="tool-textarea !min-h-[200px]" /></div>
      </div>
      <button onClick={() => setDiff(diffCode(codeA, codeB))} className="tool-btn mt-4">Compare</button>
      {diff && stats && (
        <div className="mt-6">
